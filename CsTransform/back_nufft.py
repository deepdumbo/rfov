...
#########################
def correct_phase_of_gridding(V, M, Kd ,zz ):
...
#     V1=V.getH()
    ...
    x0 = lsqr(V.getH(), b , iter_lim=10)
    x0=numpy.reshape(x0[0],(numpy.size(x0[0]),1),order='F')
    D = lsqr(V, W*x0 , iter_lim=10)
...
#     W = W*(E+1.0e-17)/(E*E+1.0e-17)    
    ...
    x1 =  lsqr(V, b , iter_lim=100)
    
    tmp_W =  lsqr(V1, x1[0], iter_lim=100)
...
#         
# def dirichlet(x):
#     return numpy.sinc(x/3.0 )*(0.5+0.5*numpy.cos(x/9.0 )) + 1.0j*x/3.0 # approximate the lancos interpolator
#     x = x+1e-17
...
def nufft_T(N, J, K, tol, alpha, beta):
    '''
     ...
     create the correlation matrix CSSC (diagonal dominent matrix)
     of J points 
...
#     import matplotlib.pyplot
#     matplotlib.pyplot.imshow(cssc)
#     matplotlib.pyplot.show()
    cssc = scipy.linalg.pinv(cssc)
    return cssc 
...
#         #=======================================================================
        ...
        for dimid in xrange(0,dd): # loop over dimensions
            ...
            T = nufft_T(N, J, K, st['tol'], alpha, beta) # inverse of CSSC using large N approx [J? J?]
            #==================================================================
...
#         print('phase_0',numpy.shape(phase_0))

#         zz = numpy.exp( 1.0j*gam * zz) 
#         print('shpae uu',uu[:])
#         print('shpae kk',kk[:])
#         print('shpae mm',mm[:])
#         sn_mask=numpy.ones(st['Nd'],dtype=numpy.float16)
    #########################################now remove the corners of sn############
        ...
        st['sn'][indx] =  st['sn'][indx] /tmp[indx]  #numpy.mean(st['sn'][...])      
        #########################################now remove the corners of sn############
#         st['sn']=st['sn']*sn_mask
...
#         st['p'] = correct_phase_of_gridding(st['p'], M, Kd , phase  )

        # Now doing the density compensation of Jackson ##
#         W=numpy.ones((st['M'],1),dtype = numpy.complex64)
#         k_tilt = numpy.ones((numpy.prod(Kd),1),dtype = numpy.complex64)
#         k_tilt = numpy.ones((numpy.prod(Kd),1),dtype = numpy.complex64)
...
#             W = W*(E+1.0e-17)/(E**2+1.0e-17)
        W = pipe_density(st['p'])
#         st['p'] = refine_k_density(st['p'],W, Kd)
#             import matplotlib.pyplot
...
#         st['w']  = numpy.abs(( st['p'].conj().T.dot(st['p'].dot(k_tilt)))) * 3
#         print(numpy.max(st['w'].flatten()))
#         st['w'] = st['w']/numpy.max(st['w'].flatten())/10.0
