...
def checker(input_var,desire_size):
    ...
    if dd > 1:
        if dims[0:dd] != desire_size[0:dd]:
            print(dims[0:dd])
            print(desire_size)
            print('input signal has wrong size')
    elif dd == 1:
...
#    print(kb_a,kb_m)
    if J > 1:
        ...
        sn_kaiser = numpy.ones((1,N),dtype=numpy.complex64)
#            print(sn_kaiser)
    gam = 2*numpy.pi/K;
...
#   print(X)
    ...
    X=numpy.array(X,dtype=numpy.complex64)
    sn_kaiser=numpy.array(sn_kaiser,dtype=numpy.complex64)
...
def nufft_T(N, J, K, tol, alpha, beta):
    ...
    for l1 in xrange(-L,L+1):
        for l2 in xrange(-L,L+1):
            ...
            tmp = numpy.sinc(1.0*tmp/(1.0*K/N))
...
#         print('shpae kk',kk[:])
#         print('shpae mm',mm[:])
        st['p'] = scipy.sparse.csc_matrix((numpy.reshape(uu,(numpy.size(uu),)),(numpy.reshape(mm,(numpy.size(mm),)),numpy.reshape(kk,(numpy.size(kk),)))),(M,numpy.prod(Kd)))
        #st['p']= st['p'].tocsc()

#         st['scale']=numpy.sqrt(numpy.prod(st['Kd'])*1.0 )

#         st['q'] = st['p']
        st['T'] = st['p'].conj().T.dot(st['p'])
...
#         import multiprocessing
        self.threads=4#multiprocessing.cpu_count()
...
#             Lprod = numpy.size(x)/numpy.prod(Nd)
    ...
    #        print('L=',L)
    #        print('Lprod',Lprod)
            Xk = numpy.zeros( (numpy.prod(Kd), L),dtype=numpy.complex64) # [*Kd *L]
    ...
    def Kd2Nd(self,Xk_all,weight_flag):
        ...
        x=numpy.zeros(Kd+(Lprod,),dtype=numpy.complex64)  # [*Kd *L]
...
#         return data_dev#.get()     
    def emb_fftn(self, input_x, output_dim, act_axes):
        ...
        '''
        output_x=numpy.zeros(output_dim, dtype=numpy.complex64)
...
#                 print('threads=',self.threads)
            output_x=pyfftw.interfaces.scipy_fftpack.fftn(output_x, output_dim, act_axes, threads=self.threads)
        else: 
#                 print('using OLD interface')                
    ...
    def emb_ifftn(self, input_x, output_dim, act_axes):
        ...
        output_x=input_x
        output_x=self.emb_fftn(output_x.conj(), output_dim, act_axes).conj()/numpy.prod(output_dim)
